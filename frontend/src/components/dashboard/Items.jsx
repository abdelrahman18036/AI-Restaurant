...
const Items = () => {
    const staticRestaurantData = {
      ...
      const openModal = (item = null) => {
        setEditingItem(item);
        setFormData({
          name: item ? item.name : "", // If editing, fill in the name
          cost: item ? item.cost : "", // If editing, fill in the price
        });
        setIsModalOpen(true);
      };
                ...
                {selectedCategory.meals.map((meal) => (
                  <tr key={meal.id} className="bg-[#313131] border-b border-[#C19D60]/20 hover:bg-[#3a3a3a] transition duration-300">
                    ...
                    <td className="px-6 py-4 flex justify-center space-x-2">
                      ...
                      {/* Delete Button */}
                      <button className="p-2 px-3  cursor-pointer font-semibold bg-[#b83b3b] hover:bg-[#ff4d4d] text-white rounded-md flex items-center transition duration-300">
